  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-blue-100 flex flex-col items-center justify-center">
        ...
        <main className="flex flex-col sm:flex-row w-full max-w-5xl bg-white rounded-lg shadow-xl overflow-hidden sm:h-[70vh]">
          <div className="flex flex-col w-full sm:w-1/2 h-[50vh] sm:h-full bg-gray-900 overflow-y-auto">
            <div className="flex-1 p-4 text-white">
              <textarea 
                onChange={(e) => setContent(e.target.value)} 
                placeholder="レビューしたいコードを貼り付けてください"
                className="h-full w-full bg-transparent text-white resize-none outline-none placeholder:text-gray-400" 
              />
            </div>
          ...
          {/* Geminiの返答 */}
          <div className="flex flex-col w-full sm:w-1/2 h-[50vh] sm:h-full items-center justify-center">
          <div className="p-4 overflow-y-auto w-full h-full">{geminiIsLoading ? "レビュー中・・・" : <Markdown className="markdown">{geminiResult}</Markdown>}</div>
          </div>
        </main>
      </div>
    </>
